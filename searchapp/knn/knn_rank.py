...
def buildDF(query_list, inverIndex, need_topics, content_map):
    ...
    for word in query_list:
        for doc in inverIndex[word]["docs"]:
            ...
            if need_topics:
                ...
                topic = content_map[split_doc_id]["topics"]
                if topic is None:
                    continue
...
def rank(query, amount, corpus, need_topics, inverIndex, content_map):
    ...
    df = buildDF(query_list, inverIndex, need_topics, content_map)
    rows, columns = df.shape
    rankedDictList = []

    for index, row in df.iterrows():
        docVec = row.to_numpy()
        score = 1 - cosine(query_vec, docVec)
        docId = row.name.split(".")[0]
        newDict = {"docId": docId, "score": score} 
